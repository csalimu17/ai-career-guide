...
export default function SubscriptionsPage() {
  ...
  const filteredSubs = allUsers?.filter((sub) => {
    ...
    return (
      !searchTerm ||
      sub.email?.toLowerCase()?.includes(normalized) ||
      sub.firstName?.toLowerCase()?.includes(normalized) ||
      sub.lastName?.toLowerCase()?.includes(normalized)
    )
  })
                ...
                <TableCell>
                  <div className="flex items-center gap-2 text-slate-500">
                    ...
                    <span className="text-xs font-medium">
                      {sub.currentPeriodEnd?.toDate ? sub.currentPeriodEnd.toDate().toLocaleDateString() : (sub.currentPeriodEnd ? new Date(sub.currentPeriodEnd).toLocaleDateString() : "N/A")}
                    </span>
                  </div>
                </TableCell>
