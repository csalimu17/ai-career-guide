...
export default function UserManagementPage() {
  ...
  const handleSyncUsers = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Checked ${payload.inspectedCount} auth users, created ${payload.createdCount} records, refreshed ${payload.updatedCount}, and removed ${payload.deletedCount || 0} stale profiles.`,
      })
    } catch (error) {
  ...
  useEffect(() => {
    if (!user || !adminRecord || hasAutoSyncedRef.current) return

    const syncKey = "admin_users_last_reconcile_at"
    const lastSync = Number(sessionStorage.getItem(syncKey) || "0")
    const now = Date.now()
    const fiveMinutes = 5 * 60 * 1000

    if (now - lastSync < fiveMinutes) return

    hasAutoSyncedRef.current = true
    sessionStorage.setItem(syncKey, String(now))
    void handleSyncUsers()
  }, [adminRecord, handleSyncUsers, user])
