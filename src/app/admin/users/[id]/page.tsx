...
export default function UserDetailPage() {
  ...
  const rawNotes = profile?.adminNotes;
  const supportNotes = (Array.isArray(rawNotes) ? [...rawNotes] : [])
    .sort((a, b) => {
            ...
            <CardContent className="space-y-5 px-4 py-4 sm:px-6 sm:py-6">
              <div className="flex items-center gap-4">
                <Avatar className="h-16 w-16 rounded-2xl bg-slate-100 ring-1 ring-black/5">
                  ...
                  <AvatarFallback className="bg-slate-100 text-xs font-bold text-slate-400">
                  ...
                  {supportNotes.length ? (
                    <div className="space-y-4">
                      {supportNotes.map((note, index) => {
                        const createdAt = note.createdAt?.toDate ? note.createdAt.toDate() : (note.createdAt ? new Date(note.createdAt) : null);
                        return (
                          <div key={note.id || index} className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:bg-slate-50">
                            <div className="flex items-start justify-between gap-4">
                              <p className="text-sm font-medium leading-relaxed text-slate-700">{note.body || note.content || "Untitled internal note"}</p>
                              <p className="shrink-0 text-[10px] font-black uppercase tracking-widest text-slate-400">
                                {createdAt ? createdAt.toLocaleDateString() : "RECENT"}
                              </p>
                            </div>
                            <div className="mt-3 flex items-center gap-2">
                              <p className="text-[10px] font-black uppercase tracking-widest text-indigo-600">
                                BY {note.authorEmail || note.createdBy || "SYSTEM"}
                              </p>
                            </div>
                          </div>
                        );
