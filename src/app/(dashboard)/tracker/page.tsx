...
export default function TrackerPage() {
  ...
  const stats = [
    { label: "Total tracked", value: jobs?.length || 0, color: "text-slate-900" },
    { label: "Saved", value: getStatusCount(jobs, "saved"), color: "text-amber-800" },
    { label: "Started", value: getStatusCount(jobs, "started"), color: "text-sky-700" },
    { label: "Applied+", value: (jobs || []).filter((job) => ["applied", "interviewing", "offer"].includes(job.status)).length, color: "text-blue-700" },
  ]
  ...
  return (
    <div className="mx-auto max-w-[1440px] px-4 pb-24 pt-8 md:px-10 lg:pt-12">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        ...
        <div className="absolute top-[20%] -right-[10%] h-[50%] w-[40%] rounded-full bg-blue-500/5 blur-[150px]" />
        <div className="absolute -bottom-[10%] left-[20%] h-[40%] w-[60%] rounded-full bg-slate-900/[0.02] blur-[100px]" />
      </div>

      <div className="space-y-12">
        {/* Header Section */}
        <section className="relative flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-[820px] space-y-5">
            <h1 className="headline-gradient-vivid max-w-6xl pb-4 text-[1.95rem] font-black leading-[1.12] tracking-tight sm:text-[2.7rem] lg:text-[3.6rem]">
              Manage your <br />
            ...
            <p className="max-w-xl text-base font-medium leading-relaxed text-slate-500 md:text-lg">
              Monitor your career momentum from saving to final outcome. Every application started in discovery lands here automatically.
            </p>
          </div>
          
          <div className="flex flex-col gap-4 sm:flex-row lg:shrink-0">
            <Button variant="outline" size="lg" className="h-16 rounded-[1.25rem] border-2 border-slate-100 bg-white/50 px-8 font-black text-slate-900 backdrop-blur-sm transition-all hover:bg-white hover:border-slate-300" asChild>
              <Link href="/jobs">Explore Market</Link>
            </Button>
            <Button
              size="lg"
              className="h-16 rounded-[1.25rem] bg-slate-900 px-8 font-black text-white shadow-2xl shadow-slate-900/20 transition-all hover:bg-slate-800"
              onClick={() => {
                resetForm()
                setIsDialogOpen(true)
              }}
      ...
      <div className="grid grid-cols-2 gap-4 md:gap-6 xl:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.label} className="surface-card group relative overflow-hidden p-6 md:p-8">
            <p className="text-[0.68rem] font-black uppercase tracking-[0.25em] text-slate-400 mb-1">{stat.label}</p>
      ...
      {isLoading ? (
              ...
              <Card key={job.id} className="surface-card group flex h-full flex-col overflow-hidden border-none transition-all duration-300 hover:-translate-y-2">
                <CardHeader className="space-y-4 p-8 pb-4">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary" className={cn("rounded-xl px-3 py-1 text-[0.65rem] font-black uppercase tracking-widest shadow-sm border-none leading-none", statusConfig.chipClassName)}>
                ...
                <CardContent className="flex-1 space-y-6 p-8 pt-0">
                  <div className="flex flex-wrap gap-2">
                    <Badge variant="outline" className={cn("rounded-full border-none px-3.5 py-1 text-[0.65rem] font-black uppercase tracking-wide", sourceConfig.badgeClassName)}>
                      {sourceConfig.label}
                    </Badge>
                    <Badge variant="outline" className="rounded-full border-slate-100 bg-white px-3.5 py-1 text-[0.65rem] font-black uppercase tracking-wide text-slate-400">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <p className="text-[0.68rem] font-black uppercase tracking-widest text-slate-300">Location</p>
                    ...
                    <div className="space-y-1">
                      <p className="text-[0.68rem] font-black uppercase tracking-widest text-slate-300">Last activity</p>
                      <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
                  ...
                  {job.resumeName ? (
                    <div className="rounded-2xl border border-indigo-50 bg-[#F5F7FF] p-4 shadow-inner">
                      <p className="text-[0.68rem] font-black uppercase tracking-widest text-indigo-400 leading-none">Anchor CV version</p>
                  ...
                  <div className="space-y-3">
                    <p className="text-[0.65rem] font-black uppercase tracking-widest text-slate-400">Update pipeline status</p>
                ...
                <CardFooter className="grid grid-cols-1 gap-3 p-8 pt-0 sm:grid-cols-2">
                  <Button
                    size="sm"
                    className="tap-bounce h-11 rounded-xl bg-slate-900 px-4 text-[0.65rem] font-black uppercase tracking-widest text-white shadow-lg shadow-slate-900/10 transition-all hover:bg-slate-800"
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="tap-bounce h-11 rounded-xl border-2 border-slate-100 bg-white px-4 text-[0.65rem] font-black uppercase tracking-widest text-slate-900 transition-all hover:border-slate-300 hover:bg-slate-50"
                    asChild
                  >
        ...
        <div className="flex flex-col items-center justify-center space-y-12 rounded-[4rem] bg-white/40 backdrop-blur-sm px-6 py-28 text-center border border-white/80 shadow-[0_32px_80px_-40px_rgba(0,0,0,0.08)]">
          <div className="relative w-full max-w-[380px] aspect-[4/3] animate-in zoom-in-95 duration-1000">
            <Image 
              src="/illustrations/tracker-empty-modern.png" 
          ...
          <div className="max-w-md space-y-5">
            <h2 className="text-4xl font-black tracking-tight text-slate-900 leading-[0.95]">
              {searchTerm ? "No roles matched" : "Pipeline is silent"}
            </h2>
            <p className="text-base font-medium leading-relaxed text-slate-400 italic">
              {searchTerm
                ...
                : "Your tracker is empty. Roles you start from discovery land here automatically, or you can record external applications manually."}
            </p>
          </div>
          <div className="flex flex-col gap-4 sm:flex-row">
            {searchTerm ? (
              <Button 
                variant="outline" 
                className="tap-bounce h-14 rounded-2xl border-2 border-slate-100 px-8 font-black text-slate-900 transition-all hover:bg-slate-50 hover:border-slate-300 shadow-sm" 
              ...
              <>
                <Button 
                  className="tap-bounce h-14 rounded-2xl bg-slate-900 px-10 font-black text-white shadow-xl shadow-slate-900/10 transition-all hover:bg-slate-800" 
                  asChild
                >
                ...
                <Button 
                  variant="outline" 
                  className="tap-bounce h-14 rounded-2xl border-2 border-slate-100 px-8 bg-white font-black text-slate-900 transition-all hover:bg-slate-50 hover:border-slate-300 shadow-sm" 
            ...
            <div className="mt-10 grid gap-8 pb-4">
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Target Company</label>
                  <Input 
                ...
                <div className="space-y-2">
                  <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Position Role</label>
                  <Input 
                    value={formData.role} 
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Information Source</label>
                ...
                <div className="space-y-2">
                  <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Current Phase</label>
              ...
              <div className="space-y-2">
                <label className="text-[0.68rem] font-black uppercase tracking-[0.2em] text-slate-400">Listing URL (Optional)</label>
                <Input 
              ...
              <div className="grid gap-6 md:grid-cols-3">
                <div className="space-y-2">
                  <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Setting</label>
                ...
                <div className="space-y-2">
                  <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Agreement</label>
                ...
                <div className="space-y-2">
                  <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Anchor CV</label>
              ...
              <div className="space-y-2">
                  <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Primary Location</label>
                  <Input 
              ...
              <div className="space-y-2">
                <label className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-slate-400">Research & Notes</label>
                <Textarea 
