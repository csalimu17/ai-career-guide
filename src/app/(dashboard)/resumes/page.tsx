...
export default function ResumesPage() {
  ...
  const confirmCreateNew = () => {
    ...
    router.push(`/cv-editor?new=true&name=${encName}&returnTo=${encodeURIComponent("/resumes")}`)
        ...
        <section className="section-shell relative overflow-hidden border-none p-5 sm:p-6 md:p-10">
          ...
          <div className="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div className="max-w-3xl space-y-3 pt-1">
              <h1 className="max-w-6xl pb-2 text-[1.75rem] font-black leading-[1.12] tracking-tight sm:text-[2.3rem] lg:text-[3.1rem]">
            ...
            <Button
              ...
              className={cn(
                ...
                "w-full justify-start bg-gradient-to-r from-violet-600 via-fuchsia-500 to-orange-400 px-5 text-white shadow-[0_22px_50px_-28px_rgba(124,58,237,0.52)] hover:-translate-y-0.5 hover:shadow-[0_30px_62px_-30px_rgba(124,58,237,0.6)] md:w-auto md:justify-center"
              )}
            >
          ...
          <div className="flex flex-col items-center justify-center space-y-10 rounded-[3.5rem] bg-[#FAFBFD]/50 px-6 py-24 text-center border border-slate-50 italic font-medium">
            ...
            <div className="max-w-md space-y-4">
              <h2 className="text-3xl font-black tracking-tight text-slate-900 leading-[0.9]">
                {searchQuery ? "No versions matched" : "Zero active CVs"}
              </h2>
              <p className="text-base font-medium leading-relaxed text-slate-400">
                {searchQuery 
                  ? `No resumes match your search "${searchQuery}". Try broadening your terms or clear the search to see your full collection.`
                  : "Every application starts with a story. Create your first masterpiece to begin tracking your professional evolution."
                }
              </p>
            </div>
            {!searchQuery && (
    ...
    <Card className="group relative flex h-full flex-col overflow-hidden rounded-[1.45rem] border-muted/20 bg-card/40 backdrop-blur-sm transition-all duration-300 hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5 md:rounded-2xl">
      ...
      <CardFooter className="mt-auto p-4 pt-3 md:p-5 md:pt-4">
        <div className="flex items-center justify-between w-full gap-3">
          ...
          <Link
            href={`/cv-editor?id=${resume.id}&returnTo=${encodeURIComponent("/resumes")}`}
  ...
  return (
    <div className="group rounded-[1.5rem] border border-white/70 bg-white/82 p-4 shadow-[0_24px_55px_-42px_rgba(15,23,42,0.28)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/20 hover:shadow-[0_30px_65px_-42px_rgba(124,58,237,0.24)] md:rounded-[1.7rem] md:p-5">
      <div className="flex items-start gap-3">
        <Link href={`/cv-editor?id=${resume.id}&returnTo=${encodeURIComponent("/resumes")}`} className="min-w-0 flex-1">
          <div className="flex items-start gap-3">
