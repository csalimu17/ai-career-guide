...
const scoreTone = (score: number) => (score >= 85 ? "text-emerald-700" : score >= 70 ? "text-sky-700" : score >= 55 ? "text-amber-800" : "text-rose-700")
const scoreToneMuted = (score: number) => (score >= 85 ? "text-emerald-600/80" : score >= 70 ? "text-sky-600/80" : score >= 55 ? "text-amber-700/80" : "text-rose-600/80")
const scoreBg = (score: number) => (score >= 85 ? "bg-emerald-500/10" : score >= 70 ? "bg-sky-500/10" : score >= 55 ? "bg-amber-500/10" : "bg-rose-500/10")
const scoreBorder = (score: number) => (score >= 85 ? "border-emerald-500/20" : score >= 70 ? "border-sky-500/20" : score >= 55 ? "border-amber-500/20" : "border-rose-500/20")
...
function BentoCard({ children, className, glow = false }: { children: React.ReactNode; className?: string; glow?: boolean }) {
  return (
    <div className={cn(
      "relative overflow-hidden group rounded-[2.5rem] border border-white/85 bg-[linear-gradient(180deg,rgba(255,255,255,0.95),rgba(247,250,255,0.92))] shadow-[0_28px_80px_-48px_rgba(74,88,130,0.3)] backdrop-blur-2xl transition-all duration-500 hover:shadow-[0_30px_80px_-42px_rgba(74,88,130,0.34)]",
      glow && "border-primary/20 shadow-[0_26px_70px_-38px_rgba(113,88,255,0.22)] hover:border-primary/30",
      className
    )}>
      <div className="absolute inset-x-0 -top-px h-px bg-gradient-to-r from-transparent via-white/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(51,184,255,0.09),transparent_24%),radial-gradient(circle_at_bottom_left,rgba(255,159,110,0.08),transparent_22%)] opacity-80" />
      {children}
    </div>
  )
}
    ...
    <Link href={href} className="block group">
      <BentoCard className="h-full p-6">
        <div className="flex items-center justify-between">
          <div className={cn("icon-orb h-11 w-11 transition-transform group-hover:scale-110", color)}>
...
export default function DashboardPage() {
  ...
  const resumesQuery = useMemoFirebase(() => (!db || !uid ? null : query(collection(db, "users", uid, "resumes"), orderBy("updatedAt", "desc"), limit(20))), [db, uid])
  ...
  const dashboardEditorHref = `/cv-editor?returnTo=${encodeURIComponent("/dashboard")}`
  ...
  const checklist = [
    { label: "Resume in workspace", done: Boolean(latestResume), href: latestResume ? dashboardEditorHref : "/onboarding" },
  ...
  const primaryAction: ActionCard = !latestResume
    ...
    : latestScore < 80
    ? { title: "Raise your ATS score", description: "Your materials are close, but the latest benchmark says they can be sharper.", href: dashboardEditorHref, cta: "Refine resume", icon: Zap, variant: "primary" }
  ...
  const actionCards: ActionCard[] = [
    { title: latestResume ? "Update your CV" : "Build your first draft", description: latestResume ? "Switch templates, improve copy, and keep your strongest version current." : "Create a polished base version you can tailor role by role.", href: latestResume ? dashboardEditorHref : "/onboarding", cta: latestResume ? "Open builder" : "Build resume", icon: FileText, variant: "primary" },
  ...
  const statCards = [
    ...
    { label: "Active Pipeline", value: `${activePipelineCount}`, icon: Layers3, color: "text-sky-700", href: "/tracker", trend: { value: "Stable", up: true } },
    ...
    { label: "Offers", value: `${offerCount}`, icon: Rocket, color: "text-emerald-700", href: "/tracker" },
  ]
  ...
  if (isMobile) {
    return (
      <MobileDashboardExperience
        ...
        editorHref={dashboardEditorHref}
      />
    )
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-4 pb-24 pt-8 md:px-8">
      {/* --- Elevated Hero --- */}
      <section className="relative overflow-hidden rounded-[3rem] border border-white/85 bg-[linear-gradient(180deg,rgba(255,255,255,0.94),rgba(247,250,255,0.9))] shadow-[0_32px_90px_-44px_rgba(74,88,130,0.3)] backdrop-blur-2xl">
        ...
        <div className="flex flex-col gap-7 p-8 md:p-12">
          <div className="space-y-5">
            <div className="space-y-5">
              <h1 className="font-display max-w-6xl pb-3 text-[2rem] font-black leading-[1.12] tracking-[-0.035em] sm:text-[2.75rem] lg:text-[3.75rem]">
                <span className="bg-gradient-to-r from-blue-600 via-indigo-500 to-orange-500 bg-clip-text text-transparent pb-1">
                  Your morning career studio.
                </span>
              </h1>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="outline" className={cn(dashboardChipBase, "border-border/80 bg-white/65 backdrop-blur-sm text-muted-foreground")}>{(profile?.plan || "free").toUpperCase()} WORKSPACE</Badge>
            </div>
          </div>
      ...
      {/* --- Premium Quick Access Row --- */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:flex lg:gap-6">
        <Link 
          href={dashboardEditorHref} 
          className="group relative flex h-20 w-full items-center gap-4 rounded-full border border-slate-200 bg-white/80 px-4 transition-all hover:border-slate-950 hover:bg-white hover:shadow-xl lg:h-24 lg:w-[320px]"
        >
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-slate-950 text-white shadow-lg transition-transform group-hover:scale-110 lg:h-16 lg:w-16">
            <FileText className="h-5 w-5 lg:h-7 lg:w-7" />
          </div>
          <div>
            <p className="text-lg font-black tracking-tight text-slate-950 lg:text-xl">Builder</p>
            <p className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Resume Edits</p>
          </div>
          <ChevronRight className="ml-auto h-5 w-5 text-slate-200 transition-transform group-hover:translate-x-1 group-hover:text-slate-950" />
        </Link>
        ...
        >
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-slate-950 text-white shadow-lg transition-transform group-hover:scale-110 lg:h-16 lg:w-16">
            <Target className="h-5 w-5 lg:h-7 lg:w-7" />
          </div>
          <div>
            <p className="text-lg font-black tracking-tight text-slate-950 lg:text-xl">ATS Center</p>
            <p className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Benchmarking</p>
          </div>
          <ChevronRight className="ml-auto h-5 w-5 text-slate-200 transition-transform group-hover:translate-x-1 group-hover:text-slate-950" />
        </Link>
      </div>
                ...
                {(["command", "pipeline", "ats"] as InsightView[]).map((view) => (
                  <Button
                    ...
                    className={cn(
                      ...
                      insightView === view
                      ? "bg-slate-950 text-white shadow-[0_12px_30px_-10px_rgba(2,6,23,0.3)] hover:brightness-110"
                      : "bg-white/0 text-slate-500 hover:bg-white hover:text-slate-900 hover:shadow-sm"
                    )}
            ...
            <div className="min-h-[280px]">
              {insightView === "command" ? (
                    ...
                    {actionCards.map((action) => (
                      ...
                      >
                        ...
                        <div className="mt-4 flex items-center justify-between">
                          <span className="text-[0.68rem] font-black uppercase tracking-[0.18em] text-primary/55">
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
                  {pipelineStages.map((stage) => (
                    <Link key={stage.status} href="/tracker" className="group rounded-3xl border border-border/40 bg-[#FAFBFD] p-6 transition-all hover:border-primary/20 hover:bg-white hover:shadow-lg">
                      <Badge variant="outline" className={cn("rounded-full px-3 py-1 text-[0.68rem] font-black uppercase tracking-[0.15em] border-none", JOB_STATUS_CONFIG[stage.status].chipClassName)}>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="rounded-3xl border border-border/40 bg-gradient-to-r from-emerald-500/5 to-sky-500/5 p-6">
                      <p className="text-[0.68rem] font-bold uppercase tracking-widest text-emerald-600/70">Latest Match Score</p>
                      ...
                      <div className="mt-6 space-y-2.5">
                        <div className="flex items-center justify-between text-[0.68rem] font-bold uppercase tracking-widest text-muted-foreground">
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center">
                  <div className="relative w-full max-w-[200px] aspect-[4/3] mb-6">
                    <Image 
                      src="/illustrations/ats-empty-modern.png" 
                      alt="No ATS activity" 
                      fill 
                      className="object-contain opacity-80"
                    />
                  </div>
                  <h3 className="text-xl font-black text-primary tracking-tight">No ATS intelligence yet</h3>
                  <p className="mt-2 text-sm font-medium text-muted-foreground max-w-[280px] italic">Benchmark your resume against a job role to see matching feedback here.</p>
                  <Button
                    asChild
                    className={cn(
                      desktopActionButtonBase,
          ...
          <BentoCard className="p-8">
            ...
            <div className="space-y-4">
              {filteredJobs.length ? filteredJobs.map((job) => {
                  ...
                  <div key={job.id} className="group relative flex flex-col gap-4 overflow-hidden rounded-[2rem] border border-border/50 bg-[#FAFBFD] p-6 transition-all hover:bg-white hover:border-primary/20 hover:shadow-lg md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="min-w-0 flex-1 space-y-3">
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge variant="outline" className={cn("rounded-full px-3 py-1 text-[0.68rem] font-black uppercase tracking-[0.14em] border-none shadow-sm", JOB_STATUS_CONFIG[status].chipClassName)}>
                          {JOB_STATUS_CONFIG[status].label}
                        </Badge>
                        {source && <Badge variant="outline" className={cn("rounded-full px-3 py-1 text-[0.68rem] font-bold uppercase tracking-[0.12em] border-none bg-muted/40", JOB_SOURCE_CONFIG[source].badgeClassName)}>{JOB_SOURCE_CONFIG[source].shortLabel}</Badge>}
                      </div>
                      <div>
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-[0.68rem] font-bold uppercase tracking-widest text-muted-foreground/60">
                ...
                <div className="flex flex-col items-center justify-center rounded-[2rem] border-2 border-dashed border-border/40 bg-muted/5 p-12 text-center">
                  <div className="relative w-full max-w-[240px] aspect-[4/3] mb-6">
                    <Image 
                      src="/illustrations/tracker-empty.png" 
                      alt="No tracking activity" 
                      fill 
                      className="object-contain opacity-80"
                    />
                  </div>
                  <h3 className="text-xl font-black text-primary tracking-tight">No active tracking</h3>
                  <p className="text-sm font-medium text-muted-foreground mt-2 max-w-sm italic">Use the jobs workspace to discover roles and start your pipeline tracking.</p>
                  <Button
                    asChild
                    className={cn(
                      desktopActionButtonBase,
        ...
        <div className="mt-6 space-y-3.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button
              variant="outline"
              asChild
              className="h-auto rounded-[1.45rem] border border-slate-200 bg-white px-0 py-0 shadow-sm transition-all duration-300 active:scale-95"
            >
              <Link href={editorHref} className="flex min-h-[56px] w-full items-center gap-3 px-3 py-2 text-left">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-950 text-white shadow-md">
                  <FileText className="h-4 w-4" />
                </div>
                <div className="min-w-0">
                  <p className="text-[0.85rem] font-black tracking-tight text-slate-950">Builder</p>
                  <p className="truncate text-[0.68rem] font-black uppercase tracking-[0.12em] text-slate-400">
                    Edit resume
                  </p>
                </div>
              </Link>
            ...
            <Button
              ...
              className="h-auto rounded-[1.45rem] border border-slate-200 bg-white px-0 py-0 shadow-sm transition-all duration-300 active:scale-95"
            >
              <Link href="/ats" className="flex min-h-[56px] w-full items-center gap-3 px-3 py-2 text-left">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-950 text-white shadow-md">
                  <Target className="h-4 w-4" />
                </div>
                <div className="min-w-0">
                  <p className="text-[0.85rem] font-black tracking-tight text-slate-950">ATS Center</p>
                  <p className="truncate text-[0.68rem] font-black uppercase tracking-[0.12em] text-slate-400">
                    Benchmark
                  </p>
                </div>
        ...
        {statCards.map((stat) => (
          <Link key={stat.label} href={stat.href}>
            <BentoCard className="p-4 rounded-[1.5rem]">
               ...
               <p className="mt-4 text-[0.68rem] font-black uppercase tracking-widest text-muted-foreground">{stat.label}</p>
          ...
          {insightView === "command" ? (
            <div className="space-y-4">
              <div className="rounded-2xl bg-primary/5 p-4 border border-primary/10">
                <p className="text-[0.68rem] font-black uppercase tracking-widest text-primary/70 mb-2">Next Best move</p>
             ...
             <div className="grid grid-cols-2 gap-3">
              {pipelineStages.slice(0, 4).map((s) => (
                <Link key={s.status} href="/tracker" className="rounded-[1.4rem] border border-border/40 bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
                   <span className={cn("text-[0.68rem] font-black uppercase tracking-widest leading-none", scoreTone(s.count > 0 ? 100 : 0))}>{JOB_STATUS_CONFIG[s.status].label}</span>
                   <p className="mt-1 text-2xl font-black text-primary leading-tight">{s.count}</p>
                </Link>
              ))}
            </div>
          ) : latestReport ? (
            <div className="rounded-[1.5rem] bg-emerald-500/5 p-4 border border-emerald-500/10 text-center shadow-[0_16px_34px_-28px_rgba(16,185,129,0.3)]">
              <p className="text-[0.68rem] font-black uppercase tracking-widest text-emerald-600/70 mb-2">ATS Score</p>
            ...
            {recentJobs.length ? recentJobs.slice(0, 3).map((job) => (
              <div key={job.id} className="rounded-[1.4rem] border border-border/40 bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
                <div className="flex items-center justify-between mb-2">
                    <Badge variant="outline" className={cn("rounded-full px-2 py-0.5 text-[0.68rem] font-black uppercase border-none leading-none", JOB_STATUS_CONFIG[job.status || "saved"].chipClassName)}>{JOB_STATUS_CONFIG[job.status || "saved"].label}</Badge>
                    <span className="text-[0.68rem] font-bold text-muted-foreground/50 leading-none">{formatTime(job.updatedAt)}</span>
                </div>
