  ...
  try {
    ...
    do {
      ...
      for (const userRecord of page.users) {
        ...
        if (current.verified !== (userRecord.emailVerified || false)) {
          patch.verified = userRecord.emailVerified || false;
          changed = true;
        }
        if (current.suspended !== (userRecord.disabled || false)) {
    ...
    for (const userDoc of firestoreUsersSnapshot.docs) {
      if (!authUserIds.has(userDoc.id)) {
        deletedCount += 1;
        writer.delete(userDoc.ref);
      }
    }
