...
function GuideCard({ guide, compact = false }: { guide: GuidePost; compact?: boolean }) {
  return (
    <Link key={guide.slug} href={`/guides/${guide.slug}`} className="group block h-full">
      <article className="flex h-full flex-col rounded-[1.4rem] border border-border/70 bg-white/90 p-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/20 hover:shadow-md sm:p-6">
        <div className="flex flex-wrap items-center gap-2">
          <Badge className="border-none bg-primary/10 text-[0.65rem] font-bold uppercase tracking-[0.18em] text-primary">
            {guide.category}
          </Badge>
        ...
        <h2 className="mt-4 font-display text-[1.35rem] font-semibold leading-tight tracking-tight text-primary transition-colors group-hover:text-secondary sm:text-2xl">
          {guide.title}
        </h2>
    ...
    <div className="min-h-screen bg-[#FDFDFC]">
      ...
      <main className="pb-24">
        <section className="relative overflow-hidden border-b border-border/50 pb-14 pt-12 sm:pb-20 sm:pt-20">
          <div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(239,246,255,0.96)_0%,rgba(255,255,255,0.92)_45%,rgba(255,247,237,0.82)_100%)]" />
          <div className="app-shell relative grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
            ...
            <Link href={`/guides/${featuredGuide.slug}`} className="group rounded-[1.7rem] border border-white/80 bg-white/82 p-5 shadow-[0_30px_80px_-55px_rgba(15,23,42,0.45)] backdrop-blur sm:p-7">
              <div className="flex items-center justify-between gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary text-white">
                  <BookOpen className="h-5 w-5" />
                </div>
                <span className="text-xs font-bold uppercase tracking-[0.22em] text-muted-foreground">Featured playbook</span>
              </div>
              <h2 className="mt-6 font-display text-2xl font-semibold leading-tight tracking-tight text-primary sm:text-4xl">
                {featuredGuide.title}
              </h2>
              <p className="mt-4 text-sm leading-6 text-muted-foreground sm:text-base">{featuredGuide.excerpt}</p>
              <div className="mt-6 grid gap-2">
                {featuredGuide.takeaways.map((takeaway) => (
                  <p key={takeaway} className="flex items-start gap-2 text-sm leading-5 text-primary/82">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-secondary" />
                    {takeaway}
                  </p>
                ))}
        ...
        <section className="pb-16 sm:pb-20">
          <div className="app-shell space-y-6">
            <div className="max-w-3xl space-y-3">
              <p className="eyebrow-chip w-fit">Popular resources</p>
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                <Link
                  key={item.href}
                  href={item.href}
                  className="surface-card group flex items-center justify-between gap-4 px-5 py-4 text-sm font-bold text-primary transition-colors hover:text-secondary"
                >
                  {item.label}
                  <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
                </Link>
        ...
        <section className="pb-16 sm:pb-24">
          <div className="app-shell space-y-10">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl space-y-3">
                <p className="eyebrow-chip w-fit">Latest playbooks</p>
                <h2 className="font-display text-[1.9rem] font-semibold tracking-tight text-primary sm:text-4xl">
                ...
                return (
                  <div key={category} className="space-y-4">
                    <div className="flex items-center justify-between gap-4 border-b border-border/60 pb-3">
                      <h3 className="font-display text-2xl font-semibold tracking-tight text-primary">{category}</h3>
