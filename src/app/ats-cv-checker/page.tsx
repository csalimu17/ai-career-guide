...
import { ArrowRight, CheckCircle2, FileSearch, ListChecks, ScanSearch, Sparkles, Workflow } from "lucide-react";
      ...
      <main className="relative z-10 pb-24">
          ...
          <div className="app-shell relative grid gap-8 lg:grid-cols-[1.02fr_0.98fr] lg:items-center">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <p className="max-w-2xl text-[1rem] leading-7 text-slate-600 sm:text-[1.15rem]">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                {[
                  ...
                  <div key={item.label} className="canva-block px-4 py-4">
                    <p className="text-[0.62rem] font-bold uppercase tracking-[0.16em] text-slate-500 sm:tracking-[0.22em]">
                      {item.label}
                    </p>
            ...
            <div className="surface-card space-y-4 p-4 sm:p-6">
              <div className="surface-card relative overflow-hidden p-4 sm:p-5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-[0.65rem] font-bold uppercase tracking-[0.22em] text-slate-500">ATS Checker Snapshot</p>
                    <h2 className="mt-2 font-display text-[1.4rem] font-semibold text-slate-950 sm:text-[1.7rem]">Know what to fix before submission.</h2>
                  </div>
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary border border-primary/20 shadow-sm transition-all duration-300 hover:scale-110 hover:rotate-3">
                ...
                <div className="mt-5 grid gap-3">
                  {[
                    ...
                    <div key={item} className="flex items-start gap-3 rounded-xl border border-slate-100/50 bg-slate-50/50 px-3.5 py-3">
        ...
        <section className="pb-14 sm:pb-20">
          <div className="app-shell space-y-7">
            <div className="max-w-3xl space-y-3">
              <div className="eyebrow-chip w-fit">What this checker helps you catch</div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {checkerCards.map((card) => (
                <div key={card.title} className="group surface-card relative overflow-hidden p-6 sm:p-8 space-y-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md block">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary border border-primary/20 shadow-sm transition-all duration-300 group-hover:scale-110 group-hover:rotate-3">
                    <card.icon className="h-5 w-5" />
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-display text-[1.08rem] font-semibold text-slate-950 sm:text-[1.22rem]">{card.title}</h3>
          ...
          <div className="app-shell">
            <div className="surface-card grid gap-6 px-5 py-6 sm:px-8 sm:py-8 lg:grid-cols-[1fr_auto] lg:items-center">
              <div className="space-y-3 sm:space-y-4">
                <div className="eyebrow-chip w-fit">Explore related pages</div>
                <h2 className="font-display text-[1.9rem] font-semibold tracking-tight text-slate-950 sm:text-4xl">
                  Build, check, and tailor inside one connected search cluster.
                </h2>
                <p className="max-w-3xl text-[0.95rem] leading-relaxed text-slate-600 sm:text-base">
        ...
        <section className="pb-16 sm:pb-24">
          <div className="app-shell grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:gap-8">
            <div className="space-y-3 sm:space-y-4">
              <div className="eyebrow-chip w-fit">ATS CV checker FAQ</div>
              <h2 className="font-display text-[1.9rem] font-semibold tracking-tight text-slate-950 sm:text-4xl">
            ...
            <Accordion type="single" collapsible className="space-y-4">
              {faqItems.map((item, index) => (
                <AccordionItem key={item.question} value={`ats-checker-faq-${index}`} className="surface-card px-4 sm:px-6">
                  <AccordionTrigger className="py-4 text-left text-[0.95rem] font-semibold text-slate-900 hover:no-underline sm:py-6 sm:text-base">
                    {item.question}
                  </AccordionTrigger>
                  <AccordionContent className="pb-4 text-sm leading-relaxed text-slate-600 sm:pb-6">
                    {item.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
