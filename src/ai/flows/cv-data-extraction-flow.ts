...
function mergeResumeCandidates(base: ResumeData | null | undefined, incoming: ResumeData | null | undefined): ResumeData {
  ...
  return {
    personalDetails: {
      ...
      location: (() => {
        const leftLoc = normalizeValue(left.personalDetails?.location);
        const rightLoc = normalizeValue(right.personalDetails?.location);
        
        const leftValid = isLikelyLocation(leftLoc);
        const rightValid = isLikelyLocation(rightLoc);

        if (rightValid && !leftValid) return rightLoc;
        if (leftValid && !rightValid) return leftLoc;
        
        // If both valid or both invalid, pick the longer one but cap it to avoid long junk
        return rightLoc.length > leftLoc.length ? rightLoc : leftLoc;
      })(),
...
export async function extractCvData(input: any): Promise<CvDataExtractionOutput> {
  ...
  try {
    ...
    addLog('BRAIN', 'Executing Multimodal Analysis...');
    try {
      ...
      const res = await ai.generate({
        ...
        prompt: [
          { text: `You are a world-class professional CV analyst. Extract data exactly according to the schema.
CRITICAL RULES:
1. Contact Info: Separate phone, email, and location. DO NOT combine them.
...
3. Job Titles: MUST be short (e.g., "Software Engineer"). Do NOT place job responsibilities or bullet points inside the job title. Write responsibilities as a list of strings in the 'description' array.` },
      ...
      addLog('WARN', `Multimodal failed: ${e.message}`); 
    }
       ...
       try {
          ...
          for (const segment of segments) {
            const res = await ai.generate({
              ...
              prompt: `Extract structured resume data strictly according to schema. 
CRITICAL: 
...
Segment: ${segment}`,
       ...
       } catch (e: any) { addLog('WARN', 'Text Fallback failed.'); }
    }
    ...
    addLog('FATAL', 'Entire extraction process failed', e.message);
    return {
      personalDetails: { name: "System Error" },
      metadata: { 
        confidence: 0, 
        isWeak: true, 
