...
export async function buildJobResearchContext(input: JobResearchInput): Promise<JobResearchContext> {
  ...
  try {
    ...
    const response = await getAi().generate({
      ...
      Your goal is to infer a high-resolution profile for use in resume tailoring.
      
      GUIDELINES:
      - FAVOR OUTCOMES: When identifying responsibilities, phrase them as 'Delivering [X] by doing [Y]' rather than just '[Y]'.
      - TECH STACK: Be aggressive in identifying core tools and technical requirements (e.g., 'React', 'Kubernetes', 'IFRS compliance').
      - EXPLICIT EVIDENCE: If a full job description is provided, extract real, documented requirements.
      - INDUSTRY KNOWLEDGE: If only a title is present, use your internal knowledge of modern industry standards for that role to fill in the gaps.
      - Keep output concrete, professional, and directly useful for generating high-impact CV bullet points.
