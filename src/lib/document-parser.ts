...
export function extractLocation(text: string): string {
  ...
  for (const line of lines) {
    if (
      ...
      !/\d{7,}/.test(line) &&
      !/[.!?]$/.test(line) &&
      !/^(email|phone|tel|mobile|links?|urls?|web|portfolio|linkedin|github|summary|profile|about|objective|experience|education|skills|projects|certifications?|references?|statement|cv|resume)$/i.test(line)
    ) {
      const hasComma = line.includes(",");
      const looksLikePlace = /\b(london|manchester|birmingham|uk|united kingdom|city|county|sf|ny|ca|new york|california|france|germany|europe|asia|usa|canada|dubai|tokyo|paris|berlin)\b/i.test(line);
      
      if (looksLikePlace || (hasComma && !/\d/.test(line.split(',')[0]))) {
         // Final guard against non-location text
         if (!/\b(statement|summary|experience|education|skills|projects|certifications?|profile|objective|resume)\b/i.test(line)) {
           return line;
         }
      }
    }
