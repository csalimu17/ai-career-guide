...
export class LoopCvAdapter implements JobApiAdapter {
  ...
  async fetchJobs(params: JobFetchParams): Promise<JobListingRecord[]> {
    ...
    // Target UK if location is generic or empty
    const isGenericUk = !location || 
                         location.toLowerCase() === "united kingdom" || 
                         location.toLowerCase() === "uk"
    
    let query = keywords || ""
    if (isGenericUk && !query.toLowerCase().includes("uk")) {
      query = `${query} UK`.trim()
    }

    if (query) q.set("q", query)
    if (location && !isGenericUk) q.set("location", location)
