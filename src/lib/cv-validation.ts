...
export function isLikelyLocation(value: string | undefined = "") {
  const v = (value || "").trim();
  if (!v) return false;
  if (v.length < 2 || v.length > 60) return false; // Reduced max length, locations are usually short
  if (/@/.test(v)) return false;
  if (/[.!?]/.test(v)) return false; // Locations don't end in punctuation usually
  
  // Exhaustive list of resume headers that might be misidentified as location
  if (/^(summary|profile|professional summary|personal statement|about me|objective|experience|work experience|employment history|education|skills|core competencies|projects|certifications?|references?|curriculum vitae|resume|contacts?)$/i.test(v)) return false;
  
  // If it's a long sentence-like string without a comma, it's probably a header or summary line
  if (v.split(/\s+/).length > 4 && !v.includes(",")) return false;
  
  // Keyword-based rejection for common non-location terms often found near headers
  if (/\b(statement|summary|experience|education|skills|projects|certifications?|profile|objective|resume|history)\b/i.test(v)) {
    // Only allow if it clearly contains a location marker
    if (!/,|remote|uk|england|scotland|wales|ireland|usa|canada|australia|london|manchester|birmingham|city|county|sf|ny|ca|new york|california/i.test(v)) {
      return false;
    }
  }
