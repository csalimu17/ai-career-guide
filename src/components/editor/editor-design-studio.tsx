...
import {
  ...
  type TemplateConfig,
  type TemplateEntryVariant,
} from "@/lib/templates-config"
import { cn } from "@/lib/utils"

import { TemplateThumbnail } from "./template-thumbnail"

type ResumeStyleKey = "primaryColor" | "fontFamily" | "fontSize" | "lineHeight" | "margins" | "entryVariant"
...
const ENTRY_VARIANTS = [
  { label: "Standard", value: "standard", description: "Clean, traditional text layout." },
  { label: "Accented", value: "accented", description: "Pro side-borders for focus." },
  { label: "Outlined", value: "outlined", description: "Modern blocks with card feel." },
] as const
...
export function EditorDesignStudio({
  ...
  const entryVariant = (currentStyles.entryVariant || activeTemplate.design.entryVariant) as TemplateEntryVariant
  ...
  const renderTemplates = () => showTemplates ? (
    <Card className="overflow-hidden rounded-[2rem] border-white/80 bg-white/90 shadow-[0_28px_70px_-42px_rgba(15,23,42,0.35)]">
      <div className="border-b border-slate-100 bg-[linear-gradient(135deg,rgba(103,58,183,0.08),rgba(33,150,243,0.08)_50%,rgba(255,152,0,0.08))] px-5 py-5">
        ...
        {/* Unified customization controls in template section */}
        <div className="mt-6 flex flex-wrap items-center gap-4 rounded-[1.6rem] border border-white/50 bg-white/40 p-4 shadow-sm backdrop-blur-sm">
          <div className="flex flex-1 min-w-[240px] flex-col gap-2">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.16em] text-slate-400">Personalize this template</p>
            <div className="flex flex-wrap items-center gap-3">
              <div className="min-w-[140px]">
                <Select value={selectedFont} onValueChange={(value) => onUpdateStyle("fontFamily", value)}>
                  <SelectTrigger className="h-10 rounded-xl border-slate-200 bg-white text-[11px] font-bold shadow-sm">
              ...
              <div className="min-w-[140px]">
                <Select value={entryVariant} onValueChange={(value) => onUpdateStyle("entryVariant", value)}>
                  <SelectTrigger className="h-10 rounded-xl border-slate-200 bg-white text-[11px] font-bold shadow-sm">
                    <SelectValue placeholder="Block style" />
                  </SelectTrigger>
                  <SelectContent className="rounded-xl">
          ...
          <div className="flex flex-col items-end gap-2 pr-2">
            <Button
              ...
              className="h-9 rounded-xl border-slate-200 px-4 text-[10px] font-black uppercase tracking-[0.14em] text-slate-600 shadow-sm"
            >
      ...
      <div className={cn("grid gap-6 p-6", compact ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2")}>
  ...
  const renderStyles = () => showStyles ? (
    <Card className="overflow-hidden rounded-[2rem] border-white/80 bg-white/95 shadow-[0_28px_70px_-42px_rgba(15,23,42,0.28)]">
      ...
      {compact ? (
        <div className="space-y-4 p-4">
          ...
          <Accordion type="single" collapsible className="space-y-3">
            ...
            <AccordionItem value="blocks" className="overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-slate-50/70 px-4">
              <AccordionTrigger className="py-4 text-left hover:no-underline">
                <div className="min-w-0">
                  <p className="text-[0.68rem] font-black uppercase tracking-[0.18em] text-slate-400">Design blocks</p>
                  <p className="mt-1 truncate text-sm font-black tracking-tight text-slate-900">
                    {ENTRY_VARIANTS.find(v => v.value === entryVariant)?.label ?? "Standard"} style
                  </p>
                </div>
              </AccordionTrigger>
              <AccordionContent className="pb-4 pt-1">
                <div className="grid grid-cols-1 gap-2">
                  {ENTRY_VARIANTS.map((v) => {
                    const active = entryVariant === v.value
                    return (
                      <button
                        key={v.value}
                        type="button"
                        onClick={() => onUpdateStyle("entryVariant", v.value)}
                        className={cn(
                          "flex flex-col gap-1 rounded-xl border p-3 text-left transition-all",
                          active ? "border-slate-300 bg-white shadow-sm ring-1 ring-slate-100" : "border-slate-100 bg-slate-50/50 hover:bg-white hover:border-slate-200"
                        )}
                      >
                        <p className="text-xs font-black tracking-tight text-slate-900">{v.label}</p>
      ...
      <div className={cn("grid gap-4 p-5", compact ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2")}>
        ...
        <MetricCard label="Block style" value={ENTRY_VARIANTS.find(v => v.value === entryVariant)?.label ?? "Standard"} hint="Switch between clean text, accented borders, or modern cards.">
          <div className="grid grid-cols-3 gap-2">
            {ENTRY_VARIANTS.map((v) => {
              ...
              return (
                ...
                >
                  <span className="text-[10px] font-black uppercase tracking-tight text-slate-900">{v.label}</span>
                </button>
              )
            })}
          </div>
