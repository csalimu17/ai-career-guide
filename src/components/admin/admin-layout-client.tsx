          ...
          <header className="sticky top-0 z-40 flex min-h-14 shrink-0 items-center justify-between gap-2 border-b border-white/80 bg-white/78 px-3 py-2.5 backdrop-blur-xl sm:min-h-16 sm:gap-3 sm:px-6 sm:py-3">
            ...
            <div className="ml-auto flex shrink-0 items-center gap-2.5 sm:gap-4">
              <div className="hidden text-right md:block">
                <p className="text-[0.8rem] font-bold uppercase tracking-[0.08em] text-primary truncate max-w-[150px]">
                  {adminRecord?.fullName || user?.email || "Admin User"}
                </p>
                <p className="mt-1 text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-muted-foreground truncate max-w-[150px]">
                  {adminRecord?.roleIds?.[0]?.replace("-", " ") || "Governance Hub"}
                </p>
              </div>
